package sv.ascen2k.taskcontrol.config;
...
public class RestExceptionHandling {
    ...
    public RestExceptionHandling(MessageSource messageSource) {
    ...
    ProblemDetail handleHttpMessageNotReadableException(HttpMessageNotReadableException exception){
        return ProblemDetail.forStatusAndDetail(HttpStatus.BAD_REQUEST,"La entidad no puede ser procesada debido a datos con formato invalido.");
    }

    @ExceptionHandler(MethodArgumentNotValidException.class)
    ProblemDetail handleMethodArgumentNotValidException(MethodArgumentNotValidException exception){
        ProblemDetail problemDetail = ProblemDetail
                .forStatusAndDetail(HttpStatus.UNPROCESSABLE_ENTITY,  "La entidad no puede ser procesada porque contiene errores.");
